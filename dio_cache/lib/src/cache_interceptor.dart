...
typedef Logger = void Function(dynamic message);
...
class CacheInterceptor extends Interceptor {
  ...
  Future onRequest(RequestOptions options) async {
    ...
    if (extraOptions.forceUpdate) {
      logger?.call('[$cacheKey][${options.uri}] Update forced, cache is ignored');
      return await super.onRequest(options);
    }

    if (existing == null) {
      logger?.call('[$cacheKey][${options.uri}] No existing cache, starting a new request');
      return await super.onRequest(options);
    }

    if (!extraOptions.forceCache && existing.expiry.isBefore(DateTime.now())) {
      logger?.call('[$cacheKey][${options.uri}] Cache expired since ${existing.expiry}, starting a new request');
      return await super.onRequest(options);
    }
  ...
  Future<dynamic> onError(DioError err) {
    ...
    if (extraOptions.returnCacheOnError) {
      ...
      if (existing != null) {
        ...
        logger?.call('WARN [$cacheKey][${err.request.uri}] An error occured, but using an existing cache : ${err.error}');
  ...
  Future onResponse(Response response) async {
    ...
    if (!extras.isFromCache && requestExtra.isCached) {
      ...
      if (isValidHttpStatusCode(response.statusCode)) {
        ...
        logger?.call('[$cacheKey][${response.request.uri}] Creating a new cache entry than expires on  $expiry');
